...
export default function (pi: ExtensionAPI) {
  let phase: "working" | "answering" | undefined;

  const setPhase = (
    next: "working" | "answering",
    ctx: ExtensionContext,
    options?: { refresh?: boolean },
  ) => {
    const refresh = options?.refresh ?? false;
    if (!refresh && phase === next) return;

    phase = next;

    if (next === "answering") {
      ctx.ui.setWorkingMessage(pickRandom(responseMessages));
    } else {
      ctx.ui.setWorkingMessage(pickRandom(messages));
    }
  };

  const setWorking = (ctx: ExtensionContext, options?: { refresh?: boolean }) =>
    setPhase("working", ctx, options);

  pi.on("turn_start", async (_event, ctx) => {
    // New turn starts in working mode; force refresh so the whimsical message rotates every turn.
    setWorking(ctx, { refresh: true });
  });
  ...
  pi.on("message_update", async (event, ctx) => {
    ...
    // Assistant is producing user-visible answer text.
    if (t === "text_start" || t === "text_delta" || t === "text_end") {
      setPhase("answering", ctx);
      return;
    }

    // Ignore thinking/start/done/error/toolcall events to avoid mode jitter.
    // We switch back to working on tool_execution_* lifecycle events.
  });

  pi.on("tool_execution_start", async (_event, ctx) => {
    // Force refresh on each tool run so repeated tool-heavy turns still feel dynamic.
    setWorking(ctx, { refresh: true });
  });
